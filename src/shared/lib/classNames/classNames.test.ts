...
describe('classNames', () => {
    ...
    test('with additions & mods', () => {
        ...
        expect(classNames('class1', { hovered: true }, ['class2', 'class3'])).toBe(classes);
    });

    test('with additions & mods (2 false mode)', () => {
        const classes = 'class1 class2 class3 hovered';
        expect(classNames('class1', { hovered: true, scroll: false }, ['class2', 'class3'])).toBe(classes);
    });

    test('with additions & mods (mode = undefined)', () => {
        const classes = 'class1 class2 class3 active';
        expect(classNames('class1', { hovered: undefined, active: true }, ['class2', 'class3'])).toBe(classes);
    });
});
